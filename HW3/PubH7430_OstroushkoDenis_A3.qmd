...
We estimate average MS levels at 4 weeks for the two groups separately. It is not exactly clear if given variables `mutpost`, `mut1mos`, `mut3mos` measure observed values of MS levels, or change in MS levels since baseline. We do not have a variable that measures MS levels at baseline, only 1 week after baseline. Therefore, I will assume that MS values at week 4 measure change from baseline to week 4. Average MS levels at 4 weeks for controls is:  

\begin{align}
E[MS_{ij} | Week = 4, Group = Placebo] &= \beta_0 + \beta_1  
\end{align}

Average MS levels at 4 weeks for treated are: 

\begin{align}
E[MS_{ij} | Week = 4, Group = Group] &= \beta_0 + \beta_1 + \beta_3 + \beta_4 
\end{align}
