...
@fig-prim-outliers-plot-2: primary outcome: reading scores: some outliers that have higher than 'normal' scores in the early periods, then likely there individuals regress to the mean 
...
@fig-outliers : yes, there are outliers, which are 'natural', and we will not do anything abut them. 
...
@fig-secndary-outcome : highly skewed outcome. Consider log-10 transformation, since highest value of 10 will become 1, which is 
convenient. 

```{r }
#| label: fig-secndary-outcome 
#| fig-cap: "Some outliers in the early observation periods" 
#| fig-width: 12
#| fig-height: 8
grid.arrange(
...
*Suggest using GEE since our primary predictor is cluster invariant*
...
**GEE might not be useful since we just wont detect any significant effects to intepret** 
...
data. We wtill probably will not find any interesting fixed effects, but will be able to show that there is significant amount of 
...
This correaltion structure looks exchangable to me. 
...
won’t have spent much time talking about this yet when you are writing your SAP.}
