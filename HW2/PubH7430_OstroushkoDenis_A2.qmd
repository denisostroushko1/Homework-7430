...
intercept_poisson = exp(glm_1$coefficients["(Intercept)"])
intercept_poisson = glm_1$coefficients["(Intercept)"]

ggplot(data = resid_data, 
       mapping = aes(x = fitted, 
                     y = outcome)) + 
  geom_point() + 
  geom_line(mapping = aes(x = fitted, y = fitted)) +
  geom_line(mapping = aes(x = fitted, y = fitted + 1.96 * sqrt(fitted * disp))) + 
  geom_line(mapping = aes(x = fitted, y = fitted - 1.96 * sqrt(fitted * disp)))
...
We may suggest centering the baseline MS levels, and other methods to address these issues. 
